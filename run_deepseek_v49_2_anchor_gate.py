...
def v49_1_promote_top_candidate_when_safe(match_result: Dict[str, Any]) -> Dict[str, Any]:
    """
...
def hierarchical_class_match(
    ...
    # 再新增 v49.1：known_class_result 为空时，检查最高候选能否安全提升。
    result = v49_1_promote_top_candidate_when_safe(result)
    return result


# ==================== v49.2: candidate confirmation gate + weak-known veto ====================
# 目的：
# v49.1 解决了“最高候选没有同步为最终预测”的问题，但在测试结果中也暴露出
# 类别内未知样本被高分已知候选拉回闭集的假阳性问题，尤其是：
# - 未知驱逐舰 -> 阿利·伯克级驱逐舰
# - 未知护卫舰 -> 阿利·伯克级驱逐舰 / 独立级濒海战斗舰
#
# v49.2 的思路不是继续简单调高/调低 confidence 阈值，而是在“候选提升”前增加
# 舰级确认门槛：最高候选除了分数足够，还必须具备该舰级的强锚点特征。
# 同时对少数已经被补全为 known_class、但缺少强锚点的结果做保守回退，降低 open-set 假阳性。

V49_2_PROMOTE_CONF = 0.74
V49_2_PROMOTE_MARGIN = 0.08
V49_2_PROMOTE_EVIDENCE = 3
V49_2_PROMOTE_MAX_CONFLICT = 1

V49_2_PROMOTE_OPENSET_CONF = 0.88
V49_2_PROMOTE_OPENSET_MARGIN = 0.12
V49_2_PROMOTE_OPENSET_EVIDENCE = 4
V49_2_PROMOTE_OPENSET_MAX_CONFLICT = 0

# 缺少强锚点时，低于该置信度的已知类结果会回退为类别内未知。
# 只作用于容易发生 open-set 假阳性的舰级，避免过度影响其它类别。
V49_2_WEAK_ANCHOR_VETO_CONF = {
    "阿利·伯克级驱逐舰": 0.90,
    "独立级濒海战斗舰": 0.86,
}


def v49_2_text_compact(x: Any) -> str:
    try:
        return compact_key(x)
    except Exception:
        return re.sub(r"\s+", "", str(x or "")).lower()
...
def v49_2_observed_to_text(observed: Dict[str, Dict[str, Any]]) -> str:
    """把原始文本和抽取槽位拼成一个可检索文本，只用于规则锚点判断。"""
    parts: List[str] = []
    try:
        meta = observed.get("_META") or {}
        if isinstance(meta, dict):
    ...
    try:
        for _group, slots in (observed or {}).items():
            if not isinstance(slots, dict):
                continue
            for _slot, val in slots.items():
                if isinstance(val, list):
...
def v49_2_has_any(text: str, cues: List[str]) -> bool:
    ck = v49_2_text_compact(text)
    return any(v49_2_text_compact(c) in ck for c in cues)


def v49_2_anchor_report(label: str, category: str, observed: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    """
    判断某个已知舰级是否具备足够强的确认锚点。

    返回字段：
    - passed: 是否通过强锚点确认
    - exact_name: 是否明确出现舰级名称/常见缩写
    - matched_groups: 命中的锚点组
    - reason: 便于调试的原因
    """
    text = v49_2_observed_to_text(observed)
    label = clean_text(label)
    category = clean_text(category)

    if not label:
        return {"passed": False, "exact_name": False, "matched_groups": [], "reason": "empty_label"}

    # 明确出现舰级名称时，认为可以通过锚点确认。
    exact_name_cues = {
        "尼米兹级航空母舰": ["尼米兹", "Nimitz", "CVN-68"],
        "提康德罗加级巡洋舰": ["提康德罗加", "Ticonderoga", "CG-47"],
        "阿利·伯克级驱逐舰": ["阿利伯克", "阿利·伯克", "伯克级", "Arleigh Burke", "DDG-51"],
        "独立级濒海战斗舰": ["独立级", "Independence", "LCS-2", "濒海战斗舰"],
        "黄蜂级两栖攻击舰": ["黄蜂级", "Wasp", "LHD-1"],
        "圣安东尼奥级两栖船坞运输舰": ["圣安东尼奥", "San Antonio", "LPD-17"],
        "惠德比岛级船坞登陆舰": ["惠德比岛", "Whidbey Island", "LSD-41"],
    }
    exact_name = v49_2_has_any(text, exact_name_cues.get(label, [label]))
    if exact_name:
        return {
            "passed": True,
            "exact_name": True,
    ...
    # 每个舰级的强锚点组。注意这里有意偏严格，避免泛化特征把未知类拉回已知类。
    anchor_groups = []

    if label == "阿利·伯克级驱逐舰":
        checks = {
            "aegis_or_spy": ["宙斯盾", "Aegis", "SPY-1", "SPY1", "SPY-6", "SPY6"],
            "mk41_or_90_96_vls": ["Mk41", "MK41", "MK-41", "90单元", "90-96", "96单元", "垂直发射单元90", "垂直发射单元96"],
            "bow_127mm_gun": ["127mm", "127毫米", "5英寸", "舰艏主炮", "舰艏一门主炮"],
            "ddg_surface_combatant": ["导弹驱逐舰", "防空驱逐舰", "DDG", "多用途导弹驱逐舰"],
        }
        for group, cues in checks.items():
            if v49_2_has_any(text, cues):
                anchor_groups.append(group)

        # 阿利·伯克必须至少具备：宙斯盾/SPY 或 Mk41/90-96 VLS 中的一个，
        # 再加舰艏127mm主炮/驱逐舰任务等辅助锚点。
        has_core = ("aegis_or_spy" in anchor_groups) or ("mk41_or_90_96_vls" in anchor_groups)
        passed = has_core and len(anchor_groups) >= 3
        return {
            "passed": passed,
            "exact_name": False,
            "matched_groups": anchor_groups,
            "reason": "arleigh_anchor_passed" if passed else "missing_arleigh_strong_anchors",
        }
    ...
    if label == "独立级濒海战斗舰":
        checks = {
            ...
            "stern_flight_deck": ["艉部直升机甲板", "舰尾直升机甲板", "艉部飞行甲板"],
        ...
        passed = ("trimaran" in anchor_groups) and len(anchor_groups) >= 3
        return {
            "passed": passed,
            "exact_name": False,
            "matched_groups": anchor_groups,
            "reason": "independence_anchor_passed" if passed else "missing_independence_strong_anchors",
        }

    if label == "尼米兹级航空母舰":
        checks = {
            "full_flight_deck": ["全通飞行甲板", "斜角飞行甲板"],
            "catapult_or_arresting": ["弹射器", "蒸汽弹射", "拦阻索", "拦阻装置"],
            "large_air_wing": ["舰载机联队", "大量固定翼", "固定翼舰载机"],
            "nuclear_or_supercarrier": ["核动力", "超级航母", "大型航空母舰"],
        }
        for group, cues in checks.items():
            if v49_2_has_any(text, cues):
                anchor_groups.append(group)
        passed = ("full_flight_deck" in anchor_groups) and len(anchor_groups) >= 2
        return {"passed": passed, "exact_name": False, "matched_groups": anchor_groups, "reason": "nimitz_anchor_check"}

    if label == "提康德罗加级巡洋舰":
        checks = {
            "cruiser": ["巡洋舰", "导弹巡洋舰"],
            "aegis_or_spy": ["宙斯盾", "Aegis", "SPY-1", "SPY1"],
            "vls_122": ["122单元", "122个垂发", "122垂发"],
            "two_main_guns": ["前后各一门", "舰艏和舰艉", "两门127"],
        }
        for group, cues in checks.items():
            if v49_2_has_any(text, cues):
                anchor_groups.append(group)
        passed = len(anchor_groups) >= 3
        return {"passed": passed, "exact_name": False, "matched_groups": anchor_groups, "reason": "ticonderoga_anchor_check"}

    if label == "黄蜂级两栖攻击舰":
        checks = {
            "full_flight_deck": ["全通飞行甲板", "直通飞行甲板"],
            "amphibious_assault": ["两栖攻击", "两栖攻击舰", "海军陆战队"],
            "stovl_or_helicopter": ["垂直起降", "短距起飞", "F-35B", "AV-8B", "直升机起降"],
            "well_deck": ["坞舱", "泛水坞", "LCAC", "登陆艇"],
        }
        for group, cues in checks.items():
            if v49_2_has_any(text, cues):
                anchor_groups.append(group)
        passed = ("full_flight_deck" in anchor_groups) and len(anchor_groups) >= 3
        return {"passed": passed, "exact_name": False, "matched_groups": anchor_groups, "reason": "wasp_anchor_check"}

    if label == "圣安东尼奥级两栖船坞运输舰":
        checks = {
            "lpd_or_transport": ["船坞运输舰", "两栖船坞运输舰", "LPD", "两栖运输"],
            "well_deck": ["坞舱", "泛水坞", "LCAC", "登陆艇"],
            "stern_gate": ["艉门", "尾门"],
            "troop_vehicle": ["陆战队员", "运兵", "车辆甲板", "车辆搭载"],
            "not_full_flight_deck": ["无全通飞行甲板", "没有全通飞行甲板", "非全通飞行甲板"],
        }
        for group, cues in checks.items():
            if v49_2_has_any(text, cues):
                anchor_groups.append(group)
        passed = ("well_deck" in anchor_groups) and len(anchor_groups) >= 3
        return {"passed": passed, "exact_name": False, "matched_groups": anchor_groups, "reason": "san_antonio_anchor_check"}

    if label == "惠德比岛级船坞登陆舰":
        checks = {
            "lsd_or_dock_landing": ["船坞登陆舰", "LSD", "登陆舰"],
            "well_deck": ["坞舱", "泛水坞", "LCAC", "登陆艇"],
            "stern_gate": ["艉门", "尾门"],
            "no_large_aviation": ["无大型航空设施", "无全通飞行甲板", "没有全通飞行甲板", "非全通飞行甲板"],
        }
        for group, cues in checks.items():
            if v49_2_has_any(text, cues):
                anchor_groups.append(group)
        passed = ("well_deck" in anchor_groups) and len(anchor_groups) >= 3
        return {"passed": passed, "exact_name": False, "matched_groups": anchor_groups, "reason": "whidbey_anchor_check"}

    # 未配置的舰级：不额外拦截。
    return {"passed": True, "exact_name": False, "matched_groups": [], "reason": "no_anchor_rule_for_label"}


def v49_2_promote_top_candidate_when_confirmed(
    match_result: Dict[str, Any],
    observed_attributes: Dict[str, Dict[str, Any]],
) -> Dict[str, Any]:
    """v49.2：在 v49.1 候选提升条件基础上，增加舰级锚点确认。"""
    if not isinstance(match_result, dict):
        return match_result

    if v49_1_has_known_final(match_result):
        match_result["v49_2_top_candidate_promotion"] = {
            "applied": False,
            "reason": "final_decision_already_known_class_before_v49_2",
        }
        return match_result
    ...
    candidates = match_result.get("known_class_candidates") or []
    if not isinstance(candidates, list) or not candidates:
        match_result["v49_2_top_candidate_promotion"] = {"applied": False, "reason": "no_known_class_candidates"}
        return match_result
    ...
    if not candidates:
        match_result["v49_2_top_candidate_promotion"] = {"applied": False, "reason": "no_valid_known_class_candidates"}
        return match_result
    ...
    open_set = match_result.get("open_set_result") or {}
    is_open_unknown = bool(open_set.get("is_unknown", False))
    anchor = v49_2_anchor_report(top_label, top_category, observed_attributes)
    ...
    debug_payload = {
        ...
        "category_compatible": compatible,
        "category_compatible_reason": compatible_reason,
        "anchor_passed": bool(anchor.get("passed")),
        "anchor_reason": anchor.get("reason"),
        "anchor_matched_groups": anchor.get("matched_groups", []),
        "anchor_exact_name": bool(anchor.get("exact_name")),
    }

    if top_label in V49_1_UNKNOWN_LABELS:
        match_result["v49_2_top_candidate_promotion"] = {"applied": False, "reason": "top_candidate_label_empty", **debug_payload}
        return match_result

    if not compatible:
        match_result["v49_2_top_candidate_promotion"] = {"applied": False, "reason": compatible_reason, **debug_payload}
        return match_result

    if not bool(anchor.get("passed")):
        match_result["v49_2_top_candidate_promotion"] = {
            "applied": False,
            "reason": "top_candidate_lacks_required_anchor_features",
            **debug_payload,
        }
        return match_result
    ...
    if is_open_unknown:
        safe_known = (
            top_conf >= V49_2_PROMOTE_OPENSET_CONF
            and margin >= V49_2_PROMOTE_OPENSET_MARGIN
            and evidence_count >= V49_2_PROMOTE_OPENSET_EVIDENCE
            and conflict_count <= V49_2_PROMOTE_OPENSET_MAX_CONFLICT
        )
        safety_profile = "v49_2_strict_open_set_guard"
        required = {
            "min_confidence": V49_2_PROMOTE_OPENSET_CONF,
            "min_margin": V49_2_PROMOTE_OPENSET_MARGIN,
            "min_evidence": V49_2_PROMOTE_OPENSET_EVIDENCE,
            "max_conflict": V49_2_PROMOTE_OPENSET_MAX_CONFLICT,
            "anchor_required": True,
        }
    else:
        safe_known = (
            top_conf >= V49_2_PROMOTE_CONF
            and margin >= V49_2_PROMOTE_MARGIN
            and evidence_count >= V49_2_PROMOTE_EVIDENCE
            and conflict_count <= V49_2_PROMOTE_MAX_CONFLICT
        )
        safety_profile = "v49_2_normal_guard"
        required = {
            "min_confidence": V49_2_PROMOTE_CONF,
            "min_margin": V49_2_PROMOTE_MARGIN,
            "min_evidence": V49_2_PROMOTE_EVIDENCE,
            "max_conflict": V49_2_PROMOTE_MAX_CONFLICT,
            "anchor_required": True,
        }
    ...
    match_result["category_result"] = {
        "label": top_category,
        "confidence": max(v49_1_float(old_cat.get("confidence", 0.0)), top_conf),
        "status": old_cat.get("status") or "matched_by_v49_2_confirmed_top_candidate",
        "reason": (old_cat.get("reason") or "") + " v49.2：最高已知舰级候选满足分数条件和强锚点确认，同步最终大类。",
    }
    ...
    match_result["known_class_result"] = {
        ...
        "known_status": "Known",
        "reason": (
            "v49.2：known_class_result 原为空，但最高候选同时满足置信度、候选间隔、"
            "证据数、冲突数、大类一致性和舰级强锚点确认，因此提升为已知舰级。"
        ),
    }
    ...
    match_result["open_set_result"] = {
        "is_unknown": False,
        "unknown_scope": None,
        "reason": "v49.2：最高候选通过强锚点确认，关闭 open-set。",
    }
    ...
    match_result["final_decision"] = {
        ...
        "primary_class": top_label,
        "confidence": round(top_conf, 4),
        "status": "v49_2_promoted_confirmed_top_candidate",
        "message": f"最终判定：{top_category} / {top_label}。v49.2 根据最高候选分数和强锚点确认提升为已知舰级。",
        "margin": round(margin, 4),
        "alternatives": old_alts,
    }
    ...
    match_result["v49_2_top_candidate_promotion"] = {
        "applied": True,
        "reason": "top_candidate_promoted_with_anchor_confirmation",
        "safety_profile": safety_profile,
        "required": required,
        **debug_payload,
    }
...
def v49_2_veto_weak_known_class_without_anchors(
    match_result: Dict[str, Any],
    observed_attributes: Dict[str, Dict[str, Any]],
) -> Dict[str, Any]:
    """
    对容易把未知类拉回已知类的结果做保守回退。

    只对少数高风险舰级生效：阿利·伯克级、独立级。
    如果没有明确舰级名称，也缺少强锚点，并且置信度低于阈值，则回退为同大类 category_unknown。
    """
    if not isinstance(match_result, dict):
        return match_result

    final = match_result.get("final_decision") or {}
    if not isinstance(final, dict) or final.get("result_type") != "known_class":
        match_result["v49_2_weak_known_veto"] = {"applied": False, "reason": "final_not_known_class"}
        return match_result

    label = clean_text(final.get("primary_class") or "")
    category = clean_text(final.get("primary_category") or (match_result.get("category_result") or {}).get("label") or "")
    conf = v49_1_float(final.get("confidence", (match_result.get("known_class_result") or {}).get("confidence", 0.0)))

    threshold = V49_2_WEAK_ANCHOR_VETO_CONF.get(label)
    if threshold is None:
        match_result["v49_2_weak_known_veto"] = {"applied": False, "reason": "label_not_in_veto_scope", "label": label}
        return match_result

    anchor = v49_2_anchor_report(label, category, observed_attributes)
    if bool(anchor.get("passed")):
        match_result["v49_2_weak_known_veto"] = {
            "applied": False,
    ...
    if conf >= threshold:
        match_result["v49_2_weak_known_veto"] = {
            "applied": False,
            "reason": "confidence_high_enough_even_without_anchor",
            "label": label,
            "confidence": round(conf, 4),
            "threshold": threshold,
            "anchor": anchor,
    ...
    old_known = match_result.get("known_class_result") or {}
    old_open = match_result.get("open_set_result") or {}

    match_result["known_class_result"] = None
    match_result["open_set_result"] = {
        "is_unknown": True,
        "unknown_scope": f"{category}类别内未知类" if category else "类别内未知类",
        "reason": (
            f"v49.2：{label} 缺少强锚点确认，且置信度 {conf:.4f} 低于 {threshold:.2f}，"
            "为降低 open-set 假阳性，回退为类别内未知。"
        ),
    }
    match_result["final_decision"] = {
        "result_type": "category_unknown",
        ...
        "primary_class": None,
        "confidence": round(conf, 4),
        "status": "v49_2_veto_weak_known_without_anchor",
        "message": f"最终判定：{category}类别内未知类。v49.2 因缺少已知舰级强锚点，未确认 {label}。",
        "alternatives": old_final.get("alternatives", {}),
    }
    match_result["v49_2_weak_known_veto"] = {
        "applied": True,
        "reason": "weak_known_class_without_required_anchors",
        "vetoed_label": label,
        "category": category,
        "confidence": round(conf, 4),
        "threshold": threshold,
        "anchor": anchor,
        "old_final_decision": old_final,
        "old_known_class_result": old_known,
        "old_open_set_result": old_open,
    }
    return match_result


_hierarchical_class_match_v49_2_base = _hierarchical_class_match_v48_base
...
def hierarchical_class_match(
    ...
    """v49.2：v49 同步 + 最高候选强锚点确认 + 弱锚点已知类回退。"""
    # 从 v48 基线开始，避免继续使用 v49.1 的无锚点候选提升。
    result = _hierarchical_class_match_v49_2_base(class_data_path, observed_attributes)

    # 保留 v49：known_class_result 已明确时，同步 final_decision。
    result = v49_sync_final_decision_force_if_known(result)

    # 替代 v49.1：只有最高候选同时满足分数门槛和强锚点，才提升为已知舰级。
    result = v49_2_promote_top_candidate_when_confirmed(result, observed_attributes)

    # 对高风险舰级做保守回退，压低 unknown -> known 假阳性。
    result = v49_2_veto_weak_known_class_without_anchors(result, observed_attributes)
    return result
