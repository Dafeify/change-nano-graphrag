...
# ==================== v49.15: precise known-class recovery after v49.12 ====================
# 目标：继续处理 V49.12 留下的“真实已知类被过度拒识为类别内未知”问题，
# 但避免 V49.13/V49.14 中 unknown->known 反弹。
# 核心原则：
# 1) 只作用于 category_unknown 结果；
# 2) 只恢复阿利·伯克级和独立级两个当前高频拒识类；
# 3) 恢复必须依赖“独有强锚点/高置信组合证据”，不能只靠共享区分特征；
# 4) 文本出现未知/未收录/类似但不等同等开放集提示时不恢复。

def v49_15_candidate_conf(result: Dict[str, Any], class_name: str) -> float:
    candidates = result.get("known_class_candidates") or []
    best = 0.0
    for c in candidates:
        label = clean_text(c.get("label") or c.get("ship_class") or "")
        if label == class_name:
            try:
                best = max(best, float(c.get("confidence", c.get("score", 0.0)) or 0.0))
            except Exception:
                pass
    known = result.get("known_class_result") or {}
    label = clean_text(known.get("label") or known.get("ship_class") or "")
    if label == class_name:
        try:
            best = max(best, float(known.get("confidence", 0.0) or 0.0))
        except Exception:
            pass
    return best


def v49_15_has_open_set_reject_cue(text: str) -> bool:
    t = compact_key(text)
    if not t:
        return False
    cues = [
        "未知类", "类别内未知", "未知驱逐舰", "未知护卫舰", "未知舰", "未知目标",
        "新型", "未收录", "未包含", "不属于已知", "不是已知", "非已知",
        "不像已知", "不能稳定匹配", "无法稳定匹配", "相似但不完全一致",
        "类似但", "接近但", "不完全符合", "差异明显",
        "不一定是阿利伯克", "不是阿利伯克", "非阿利伯克",
        "不是独立级", "非独立级",
    ]
    return any(compact_key(c) in t for c in cues)


def v49_15_restore_arleigh_reason(text: str, result: Dict[str, Any]) -> Tuple[bool, str]:
    if v49_15_has_open_set_reject_cue(text):
        return False, "open_set_reject_cue"

    # 只在当前结果是类别内未知时恢复，不触碰已有 known_class。
    if (result.get("final_decision") or {}).get("result_type") != "category_unknown":
        return False, "not_category_unknown"

    category = v49_11_final_category(result)
    if category not in {"驱逐舰", "巡洋舰", "两栖舰"}:
        return False, "category_not_recoverable_for_arleigh"

    cand_conf = v49_15_candidate_conf(result, "阿利·伯克级驱逐舰")

    has_name = v49_11_has_any(text, [
        "阿利·伯克", "阿利伯克", "伯克级", "DDG-51", "DDG51",
        "Flight IIA", "Flight III", "伯克Flight", "伯克 flight",
    ])
    has_aegis = v49_11_has_any(text, ["宙斯盾", "Aegis", "AEGIS"])
    has_spy = v49_11_has_any(text, ["SPY-1", "SPY1", "SPY-6", "SPY6", "AN/SPY", "AN SPY"])
    has_mk41 = v49_11_has_any(text, ["Mk41", "MK41", "MK-41", "Mark 41", "马克41"])
    has_front_rear_vls = v49_11_has_any(text, [
        "前后垂发", "前后垂直发射", "前后为Mk41", "前后为MK41",
        "前后为垂发", "前后垂发区", "前后垂发阵列", "舰艏和舰艉垂发",
        "前部和后部垂发", "前后两组垂发",
    ])
    has_90_96_vls = v49_11_has_any(text, [
        "90单元", "96单元", "90-96", "90至96", "九十六单元", "九十单元",
    ])
    has_127 = v49_11_has_any(text, ["127mm", "127毫米", "5英寸", "五英寸"])
    has_hangar = v49_11_has_any(text, ["直升机机库", "双机库", "双直升机机库", "2座机库", "两个机库"])
    has_destroyer_mission = v49_11_has_any(text, ["防空、反潜和对海", "防空反潜对海", "多用途导弹驱逐舰", "区域防空驱逐舰"])

    if has_name:
        return True, "v49.15：出现阿利·伯克/DDG-51/Flight 专名，恢复为阿利·伯克级。"

    # 最安全的命名系统组合：宙斯盾/SPY + Mk41/前后垂发 + 127mm/机库/任务组合。
    if (has_aegis or has_spy) and (has_mk41 or has_front_rear_vls or has_90_96_vls) and (has_127 or has_hangar or has_destroyer_mission):
        return True, "v49.15：宙斯盾/SPY + Mk41/90-96单元或前后垂发 + 127mm/机库/任务组合证据充分，恢复为阿利·伯克级。"

    # 处理 anchor_008 这类：前后 Mk41 + 127mm + 直升机机库，虽然没有显式宙斯盾。
    if has_mk41 and has_front_rear_vls and (has_127 or has_hangar) and cand_conf >= 0.65:
        return True, "v49.15：前后 Mk41 垂发 + 127mm/直升机机库组合证据充分，恢复为阿利·伯克级。"

    # 处理 known_arleigh_burke_002 这类：宙斯盾 + 前后垂发阵列 + 舰艏主炮 + 排除航母/两栖。
    if has_aegis and has_front_rear_vls and v49_11_has_any(text, ["舰艏主炮", "主炮"]) and v49_11_has_any(text, ["没有全通", "没有弹射器", "没有拦阻", "没有坞舱", "无坞舱"]):
        return True, "v49.15：宙斯盾 + 前后垂发阵列 + 舰艏主炮，并排除航母/两栖特征，恢复为阿利·伯克级。"

    return False, "insufficient_precise_arleigh_evidence"


def v49_15_restore_independence_reason(text: str, result: Dict[str, Any]) -> Tuple[bool, str]:
    if v49_15_has_open_set_reject_cue(text):
        return False, "open_set_reject_cue"
    ...
    category = v49_11_final_category(result)
    if category not in {"护卫舰", "驱逐舰"}:
        return False, "category_not_recoverable_for_independence"

    cand_conf = v49_15_candidate_conf(result, "独立级濒海战斗舰")

    has_name = v49_11_has_any(text, ["独立级", "Independence", "LCS-2", "LCS2"])
    has_trimaran = v49_11_has_any(text, ["三体船", "三体舰", "三体结构", "三体船体", "宽体三体", "两侧支撑船体", "中央主船体"])
    has_57 = v49_11_has_any(text, ["57mm", "57毫米", "Mk110", "MK110", "57 毫米"])
    has_lcs_or_module = v49_11_has_any(text, ["LCS", "濒海战斗舰", "近海作战", "模块化任务", "任务模块", "模块化"])
    has_ram_mh60 = v49_11_has_any(text, ["RAM", "拉姆", "MH-60", "MH60", "直升机平台", "直升机甲板"])
    has_param_combo = (
        v49_11_has_any(text, ["75人", "约75人", "75 名", "75名"]) and
        v49_11_has_any(text, ["4300海里", "4300 海里", "4300"]) and
        has_57 and
        v49_11_has_any(text, ["RAM", "拉姆"]) and
        v49_11_has_any(text, ["MH-60", "MH60"])
    )

    if has_name:
        return True, "v49.15：出现独立级/LCS-2 专名，恢复为独立级濒海战斗舰。"

    if has_param_combo:
        return True, "v49.15：75人 + 4300海里 + 57mm + RAM + MH-60 参数组合高度符合独立级，恢复为独立级。"

    # 三体只是必要锚点，不再单独闭集；必须叠加 LCS/模块化/近海作战 和 57mm/RAM/MH-60 中至少一个。
    if has_trimaran and has_lcs_or_module and (has_57 or has_ram_mh60) and cand_conf >= 0.62:
        return True, "v49.15：三体结构 + LCS/模块化/近海作战 + 57mm/RAM/MH-60 组合证据充分，恢复为独立级。"

    return False, "insufficient_precise_independence_evidence"


def v49_15_precise_known_recovery_fix(result: Dict[str, Any], observed: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    text = v49_11_text(observed)

    final = result.get("final_decision") or {}
    if final.get("result_type") != "category_unknown":
        result["v49_15_precise_known_recovery_fix"] = {"applied": False, "reason": "not_category_unknown"}
        return result

    # 阿利·伯克恢复
    ok, reason = v49_15_restore_arleigh_reason(text, result)
    if ok:
        out = v49_12_set_known_class(result, "驱逐舰", "阿利·伯克级驱逐舰", reason, final.get("confidence"))
        out["final_decision"]["status"] = "v49_15_precise_known_recovery_fix"
        out["v49_15_precise_known_recovery_fix"] = {"applied": True, "class_name": "阿利·伯克级驱逐舰", "reason": reason}
        return out

    # 独立级恢复
    ok, reason2 = v49_15_restore_independence_reason(text, result)
    if ok:
        out = v49_12_set_known_class(result, "护卫舰", "独立级濒海战斗舰", reason2, final.get("confidence"))
        out["final_decision"]["status"] = "v49_15_precise_known_recovery_fix"
        out["v49_15_precise_known_recovery_fix"] = {"applied": True, "class_name": "独立级濒海战斗舰", "reason": reason2}
        return out

    result["v49_15_precise_known_recovery_fix"] = {
        "applied": False,
        "reason": "no_precise_known_recovery_triggered",
        "arleigh_reason": reason,
        "independence_reason": reason2,
    }
    return result
...
def hierarchical_class_match(
    ...
    """v49.15：在 v49.12 基础上，仅用精确强锚点恢复少量阿利·伯克/独立级拒识样本。"""
    ...
    result = v49_15_precise_known_recovery_fix(result, observed_attributes)
    return result
