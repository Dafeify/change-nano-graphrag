...
# ============================================================
# V49.20: Arleigh Burke known-class profile conflict guard
# ============================================================
# 核心思想：不使用“未知类先验”，只使用“已知类画像冲突”。
# 如果文本同时具备宙斯盾/SPY/Mk41/127mm等共享区分特征，但又出现
# 与阿利·伯克级典型画像不一致的参数/结构组合，则不应闭集成阿利·伯克级，
# 而应保留为“驱逐舰 / 类别内未知”。


def v49_20_has_any_non_negated(text: str, terms: List[str]) -> bool:
    """沿用 v49.19 的非否定匹配，用于避免“没有X”被当成正证据。"""
    return v49_19_find_non_negated(text, terms)


def v49_20_has_arleigh_explicit_name(text: str) -> bool:
    """显式名称/舷号证据。它是高权重证据，但仍可被非常强的反证阻断。"""
    return v49_20_has_any_non_negated(text, [
        "阿利·伯克", "阿利伯克", "伯克级", "DDG-51", "DDG51",
        "Flight IIA", "Flight III", "flight iia", "flight iii",
    ])


def v49_20_has_arleigh_shared_aegis_features(text: str) -> bool:
    """宙斯盾驱逐舰共享区分特征：支持驱逐舰方向，但不能单独闭集为阿利·伯克。"""
    t = str(text or "")
    has_aegis_or_spy = v49_20_has_any_non_negated(t, ["宙斯盾", "Aegis", "SPY-1", "SPY-1D", "SPY-6", "AN/SPY"])
    has_mk41 = v49_20_has_any_non_negated(t, ["Mk41", "Mk 41", "MK41", "MK 41"])
    has_127 = v49_20_has_any_non_negated(t, ["127mm", "127毫米", "127 mm"])
    return (has_aegis_or_spy and has_mk41) or (has_mk41 and has_127) or (has_aegis_or_spy and has_127)


def v49_20_has_arleigh_parameter_profile_conflict(text: str) -> Tuple[bool, List[str]]:
    """检测“像宙斯盾驱逐舰但不像阿利·伯克画像”的组合冲突。

    这里不是判断它属于哪个未知类，而是判断：已有文本与阿利·伯克的已知画像不够一致。
    因此只有在“多个参数共同出现”时才触发，避免单个 90 单元或单个 9500 吨误伤。
    """
    t = str(text or "")
    ev: List[str] = []

    # 共享正证据存在时才谈阿利·伯克画像冲突；否则不在这里处理。
    if not v49_20_has_arleigh_shared_aegis_features(t):
        return False, []

    # 显式名称/舷号强证据存在时，不因参数近似冲突轻易回退；除非后续有更强反证。
    if v49_20_has_arleigh_explicit_name(t):
        return False, ["explicit_arleigh_name_present"]

    has_len_161 = v49_11_has_any(t, [
        "舰长约161", "舰长161", "161米", "161 m", "161m", "约161米",
    ])
    has_std_7250 = v49_11_has_any(t, [
        "标准排水量约7250", "标准排水量7250", "标准7250", "7250吨", "7250 吨",
    ])
    has_full_9485 = v49_11_has_any(t, [
        "满载约9485", "满载排水量约9485", "9485吨", "9485 吨",
        "满载约9500", "满载排水量约9500", "9500吨", "9500 吨",
    ])
    has_vls_90ish = v49_11_has_any(t, [
        "90具左右", "90具", "90单元左右", "90单元", "90 个", "90个", "90座", "90管", "90具Mk41", "90单元Mk41",
    ])

    if has_len_161:
        ev.append("length_about_161m")
    if has_std_7250:
        ev.append("standard_displacement_about_7250t")
    if has_full_9485:
        ev.append("full_displacement_about_9485_9500t")
    if has_vls_90ish:
        ev.append("vls_about_90_cells")

    # 最核心的画像冲突：161m + 7250t标准/9485t满载 + 90单元级Mk41。
    # 单独任一项都不是反证，组合起来才说明“更像阿利·伯克近邻宙斯盾舰，而非可闭集阿利·伯克”。
...
def v49_20_arleigh_profile_conflict_guard(result: Dict[str, Any], observed: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    """若最终结果为阿利·伯克，但文本存在已知类画像冲突，则回退为驱逐舰类别内未知。"""
    if not isinstance(result, dict):
        return result
    final = result.get("final_decision") or {}
    if final.get("result_type") != "known_class":
        result["v49_20_arleigh_profile_conflict_guard"] = {"applied": False, "reason": "not_known_class"}
        return result

    class_name = v49_11_final_class(result)
    if class_name != "阿利·伯克级驱逐舰":
        result["v49_20_arleigh_profile_conflict_guard"] = {"applied": False, "reason": "not_arleigh", "class_name": class_name}
        return result

    text = v49_11_text(observed)
    conflict, evidence = v49_20_has_arleigh_parameter_profile_conflict(text)
    if conflict:
        new_result = v49_11_set_category_unknown(
            result,
            "驱逐舰",
            "v49.20：文本虽具备宙斯盾/SPY/Mk41/127mm等共享驱逐舰特征，但存在与阿利·伯克级已知画像不一致的参数/结构组合，按开放集近邻处理为驱逐舰类别内未知。",
        )
        new_result["v49_20_arleigh_profile_conflict_guard"] = {
            "applied": True,
            "evidence": evidence,
            "reason": "arleigh_known_profile_conflict",
        }
        return new_result

    result["v49_20_arleigh_profile_conflict_guard"] = {"applied": False, "reason": "no_profile_conflict", "evidence_checked": evidence}
    return result
...
def hierarchical_class_match(
    ...
    """v49.20：在 v49.19 基础上增加阿利·伯克已知类画像冲突保护。"""
    ...
    result = v49_20_arleigh_profile_conflict_guard(result, observed_attributes)
    result["v49_19_arleigh_unknown_neighbor_guard"] = {"version": "v49.19", "note": "arleigh recovery uses non-negated evidence matching"}
    result["v49_20_arleigh_profile_conflict_guard_version"] = "v49.20"
    return result
