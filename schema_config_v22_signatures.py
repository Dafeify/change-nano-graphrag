...
# ============================================================
# 11. v22 强匹配 / 冲突 / 开放集签名规则
# ============================================================
# 注意：这里不包含任何未知舰级名称，只包含结构、装备、任务、参数等判别特征。
# run_deepseek.py 只调用这些配置，分类规则不再继续散落写死在主程序里。

SIGNATURE_RULE_THRESHOLDS: Dict[str, float] = {
    "known_protect": 14.0,
    "known_force_correct": 18.0,
    "known_conflict_block": 10.0,
    "real_unknown": 14.0,
    "real_unknown_force": 18.0,
}

KNOWN_CLASS_STRONG_SIGNATURES: Dict[str, List[Tuple[str, float]]] = {
    "尼米兹级航空母舰": [
        ("尼米兹级", 20), ("核动力航空母舰", 14), ("超级航母", 10),
        ("10万吨", 12), ("十万吨", 12), ("满载排水量超过10万吨", 16), ("104200吨", 12),
        ("舰长332.8", 12), ("332.8米", 12), ("飞行甲板宽76.4", 10),
        ("全通飞行甲板", 8), ("斜角飞行甲板", 12), ("右舷舰岛", 8),
        ("4台蒸汽弹射器", 18), ("四台蒸汽弹射器", 18), ("C-13", 12), ("C-13-1", 14),
        ("4条拦阻索", 18), ("四条拦阻索", 18), ("4座升降机", 12),
        ("固定翼舰载机", 10), ("大量固定翼舰载机", 12), ("超过100架", 12),
        ("A4W", 12), ("A4W压水反应堆", 14), ("航母战斗群核心", 10),
    ],
    "提康德罗加级导弹巡洋舰": [
        ("提康德罗加级", 20), ("提康德罗加", 18), ("导弹巡洋舰", 16), ("宙斯盾巡洋舰", 18),
        ("美国海军现役唯一一级巡洋舰", 18), ("唯一一级巡洋舰", 14),
        ("第一种正式使用宙斯盾系统", 18), ("正式使用宙斯盾系统", 14),
        ("航母战斗群的指挥中心", 16), ("指挥舰", 12), ("舰队指挥", 12),
        ("AN/SPY-1", 12), ("SPY-1", 10),
        ("122单元", 22), ("共122单元", 24), ("122单元MK41", 26),
        ("16组八联装", 22), ("16组八联装MK41", 26), ("两组61", 18),
        ("舰首和舰尾各有", 16), ("舰艏和舰艉各有", 16), ("双主炮", 16),
        ("满载排水量约9500吨", 14), ("9480吨", 12), ("9500吨", 10),
        ("舰长172.8", 12), ("172.8m", 12), ("172.8米", 12),
        ("4×LM2500", 10), ("4xLM2500", 10), ("80000马力", 10), ("6000海里", 8),
        ("比伯克级大", 14), ("比阿利·伯克级大", 14),
    ],
    "阿利·伯克级驱逐舰": [
        ("阿利·伯克级", 20), ("阿利伯克级", 20), ("伯克级", 16),
        ("美国海军主力驱逐舰", 16), ("主力驱逐舰", 12), ("多用途导弹驱逐舰", 14),
        ("宙斯盾驱逐舰", 16), ("带宙斯盾", 10), ("SPY-1D", 16), ("AN/SPY-1D", 16),
        ("四面相控阵雷达", 12), ("四个大平板雷达", 10),
        ("Flight IIA", 18), ("FlightIIA", 18), ("IIA型", 12),
        ("96单元", 16), ("96单元MK41", 20), ("96管", 14), ("90多个", 10),
        ("舰首有一座MK45", 12), ("舰艏主炮", 8),
        ("满载排水量约9200吨", 12), ("9200吨", 10), ("9238吨", 12),
        ("舰长155.29", 10), ("155.29m", 10), ("105000马力", 8),
        ("增设了直升机库", 14), ("两座直升机库", 14), ("后面有直升机库", 10),
        ("海鹰直升机", 10), ("SH-60", 10), ("MH-60", 10), ("航母的带刀护卫", 12),
    ],
    "独立级濒海战斗舰": [
        ("独立级", 20), ("濒海战斗舰", 18),
        ("三体船", 20), ("三体结构", 20), ("三个船体", 18), ("三个船身", 18),
        ("多体结构", 16), ("中央主船体两侧", 16), ("左右好像还有支撑结构", 14),
        ("铝合金船体", 12), ("铝合金造", 10), ("任务模块", 12), ("模块化任务", 12),
        ("航速45", 12), ("45-50节", 14), ("最高航速超过45节", 16),
        ("舰长127.6", 10), ("舰宽31.6", 10), ("满载排水量2784吨", 10), ("2800吨", 8),
        ("57毫米", 12), ("57mm", 12), ("RAM防空导弹", 8), ("大型艉部直升机", 10),
        ("可同时操作两架直升机", 10), ("2架MH-60", 10),
    ],
    "黄蜂级两栖攻击舰": [
        ("黄蜂级", 20), ("两栖攻击舰", 18), ("LHD", 16), ("小航母", 12),
        ("全通飞行甲板", 12), ("全通式飞行甲板", 12), ("一整块飞行甲板", 10),
        ("垂直起降战斗机", 14), ("AV-8B", 14), ("F-35B", 14), ("STOVL", 14),
        ("海军陆战队远征部队", 14), ("舰长253.2", 12), ("253.2米", 12),
        ("满载排水量41150吨", 14), ("41150吨", 14), ("四万多吨", 10),
        ("大型坞舱门", 10), ("3艘LCAC", 14), ("三艘LCAC", 14),
        ("未观察到弹射器和拦阻索", 12), ("没有弹射器和拦阻索", 12),
    ],
    "圣安东尼奥级两栖船坞运输舰": [
        ("圣安东尼奥级", 20), ("圣安东尼奥", 18),
        ("两栖船坞运输舰", 20), ("船坞运输舰", 16), ("LPD", 16),
        ("封闭式桅杆", 14), ("一体化桅杆", 14), ("先进的封闭式桅杆", 16),
        ("大型封闭式上层建筑", 14), ("大型箱形隐身化上层建筑", 16),
        ("舰长208m", 12), ("舰长208米", 12), ("满载排水量25300吨", 14), ("25300吨", 12),
        ("MV-22", 14), ("鱼鹰", 14), ("倾转旋翼机", 14),
        ("车辆甲板", 12), ("车辆甲板面积2230", 14), ("货舱容积962", 14),
        ("720名海军陆战队员", 14), ("2艘LCAC", 14), ("两艘LCAC", 14),
        ("坞舱可容纳2艘LCAC", 16), ("运兵和装备", 10),
    ],
    "惠德比岛级船坞登陆舰": [
        ("惠德比岛级", 20), ("惠德比", 18),
        ("船坞登陆舰", 20), ("船坞登陆", 16), ("LSD", 16),
        ("大型井围甲板", 18), ("井围甲板", 16), ("大型泛水坞舱", 18), ("大型坞舱", 14),
        ("专为搭载LCAC", 16), ("专为LCAC设计", 16),
        ("4艘LCAC", 18), ("四艘LCAC", 18), ("可容纳4艘LCAC", 18),
        ("21艘LCM-6", 14), ("舰长185.8", 12), ("舰长186", 10), ("185.8m", 12),
        ("满载排水量16100吨", 14), ("16000吨", 10), ("627名海军陆战队员", 12),
        ("无机库", 12), ("没有直升机库", 12), ("飞行甲板支持直升机起降但无机库", 16),
        ("登陆艇投送", 14), ("运输登陆装备", 10),
    ],
}

KNOWN_CLASS_CONFLICT_SIGNATURES: Dict[str, List[Tuple[str, float]]] = {
    "尼米兹级航空母舰": [("没有弹射器", 14), ("无弹射器", 14), ("没有拦阻索", 14), ("无拦阻索", 14), ("坞舱", 10), ("艉门", 10), ("LCAC", 10), ("两栖攻击舰", 10), ("船坞登陆舰", 12), ("三体船", 12)],
    "提康德罗加级导弹巡洋舰": [("不像巡洋舰", 16), ("不是巡洋舰", 16), ("导弹驱逐舰", 10), ("护卫舰", 12), ("巡防舰", 12), ("反潜护卫舰", 14), ("三千吨级", 12), ("K-VLS", 12), ("CODLOG", 12), ("Mk48", 12), ("OPS-24", 12), ("三体船", 14)],
    "阿利·伯克级驱逐舰": [("重型四角格子桅", 16), ("格子桅", 12), ("舰桥结构高大", 14), ("无机库", 14), ("没有直升机库", 14), ("90单元", 12), ("122单元", 18), ("16组八联装", 18), ("双主炮", 18), ("比伯克级大", 16), ("指挥舰", 12), ("OPS-24", 14), ("OYQ-9", 14), ("Mk48", 14), ("K-VLS", 14), ("CODLOG", 14), ("护卫舰", 14), ("巡防舰", 14), ("反潜为主", 14), ("三体船", 16)],
    "独立级濒海战斗舰": [("不是三体船", 18), ("没有三体船", 18), ("单体船", 12), ("普通单体船", 16), ("直升机机库", 14), ("76毫米", 12), ("127毫米", 12), ("Mk41", 12), ("Mk48", 14), ("K-VLS", 14), ("CODLOG", 14), ("OPS-24", 14), ("反潜护卫舰", 16), ("巡防舰", 14), ("大型导弹驱逐舰", 16)],
    "黄蜂级两栖攻击舰": [("弹射器", 12), ("拦阻索", 12), ("传统弹射型航母", 16), ("核动力航空母舰", 16), ("10万吨", 16), ("船坞登陆舰", 16), ("LSD", 16), ("井围甲板", 14), ("4艘LCAC", 14), ("两栖船坞运输舰", 12)],
    "圣安东尼奥级两栖船坞运输舰": [("全通飞行甲板", 14), ("两栖攻击舰", 16), ("LHD", 16), ("STOVL", 14), ("AV-8B", 14), ("F-35B", 14), ("3艘LCAC", 12), ("4艘LCAC", 16), ("船坞登陆舰", 16), ("LSD", 16), ("井围甲板", 16), ("无机库", 12)],
    "惠德比岛级船坞登陆舰": [("两栖船坞运输舰", 16), ("LPD", 16), ("圣安东尼奥", 18), ("封闭式桅杆", 14), ("一体化桅杆", 14), ("大型封闭式上层建筑", 14), ("2艘LCAC", 14), ("MV-22", 14), ("鱼鹰", 14), ("全通飞行甲板", 16), ("两栖攻击舰", 16), ("AV-8B", 14), ("核动力航空母舰", 16)],
}

REAL_UNKNOWN_CATEGORY_SIGNATURES: Dict[str, List[Tuple[str, float]]] = {
    "驱逐舰": [("大型防空导弹驱逐舰", 18), ("大型导弹驱逐舰", 16), ("重型四角格子桅", 18), ("四角格子桅", 18), ("重型格子桅", 18), ("老式的格子桅", 18), ("格子桅", 12), ("舰桥结构较为庞大", 14), ("舰桥结构高大", 14), ("无机库", 16), ("没有直升机库", 16), ("未观察到直升机库", 14), ("90单元", 14), ("共90单元", 16), ("弹道导弹防御", 10), ("反导", 10)],
    "护卫舰": [("中型通用护卫舰", 18), ("反潜护卫舰", 18), ("通用护卫舰", 14), ("巡防舰", 14), ("现代巡防舰", 16), ("单体护卫舰", 14), ("反潜为主", 16), ("普通单体船", 12), ("76毫米", 10), ("OPS-24", 16), ("OYQ-9", 16), ("Mk48", 16), ("Mk41和Mk48", 18), ("两种垂直发射系统", 18), ("拖曳声呐", 16), ("拖曳阵列声呐", 16), ("反潜直升机", 14), ("直升机机库", 12), ("三千吨级", 16), ("三千多吨", 16), ("3650吨", 14), ("122m", 14), ("16单元", 12), ("2组八联装垂直发射", 14), ("140人", 10), ("4500海里", 10), ("柴电燃", 14), ("CODLOG", 16)],
}

REAL_UNKNOWN_CATEGORY_CONFLICTS: Dict[str, List[Tuple[str, float]]] = {
    "驱逐舰": [("阿利·伯克级", 20), ("Flight IIA", 18), ("96单元", 16), ("两座直升机库", 16), ("提康德罗加", 20), ("巡洋舰", 12), ("122单元", 18), ("双主炮", 16), ("护卫舰", 12), ("巡防舰", 12), ("反潜为主", 14)],
    "护卫舰": [("独立级", 20), ("濒海战斗舰", 18), ("三体船", 20), ("三体结构", 20), ("阿利·伯克级", 20), ("Flight IIA", 18), ("96单元", 16), ("宙斯盾驱逐舰", 16), ("提康德罗加", 20), ("导弹巡洋舰", 18), ("122单元", 18), ("两栖攻击舰", 18), ("船坞登陆舰", 18), ("坞舱", 12)],
}
