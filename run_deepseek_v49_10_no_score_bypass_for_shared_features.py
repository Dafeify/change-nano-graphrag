...
def v49_3_open_set_anchor_report(
    ...
    # v49.10：取消 open-set 候选提升中的纯分数绕过。
    # 原因：V49.7/V49.9 剩余的 unknown->known 主要来自共享区分特征被高分误当成闭集确认，
    # 例如“相控阵+垂发+舰艏主炮”只能支持驱逐舰方向，不能直接确认阿利·伯克级；
    # “57mm/模块化/直升机甲板”也不能在缺少“三体船”等核心锚点时直接确认独立级。
    # 因此 open-set 状态下要提升为这两个已知舰级，必须走下面的具体锚点检查，
    # 不再允许 very_strong_candidate_score_bypass。
    strong_score_bypass = False
...
def hierarchical_class_match(
    ...
    """v49.10：基于 v49.7，取消 open-set 候选提升中的纯分数绕过，防止共享区分特征误闭集。"""
