...
# ==================== v49.8: shared-feature closed-set guard ====================
# 目的：
# - v49.7 只修 category_unknown 的大类边界，但部分 open-set 样本仍会被共享区分特征拉成 known_class。
# - 典型错误：未知驱逐舰具备“相控阵+垂发+舰艏主炮”，但这些只是驱逐舰方向共享区分特征，
#   不能直接闭集确认为“阿利·伯克级驱逐舰”。
# - v49.8 只做“弱闭集结果回退为类别内未知”，不尝试恢复 known_class。
# - 回退后再交给 v49.7 的 category-only 五类证据去修大类。

V49_8_GENERAL_LOW_CONF_DEMOTE = 0.60

V49_8_GUARDED_KNOWN_CLASSES = {
    "阿利·伯克级驱逐舰": {
        "category": "驱逐舰",
        "demote_below_conf": 0.88,
        "exact": ["阿利伯克", "阿利·伯克", "伯克级", "Arleigh Burke", "DDG-51", "DDG51"],
        "groups": {
            "aegis_or_spy": ["宙斯盾", "Aegis", "SPY-1", "SPY1", "SPY-6", "SPY6", "AN/SPY"],
            "mk41_or_90_96_vls": ["Mk41", "MK41", "MK-41", "90单元", "96单元", "90-96", "90至96"],
            "bow_127mm_gun": ["127mm", "127毫米", "5英寸", "舰艏主炮", "舰首主炮"],
            "ddg_or_flight": ["导弹驱逐舰", "防空驱逐舰", "多用途导弹驱逐舰", "DDG", "Flight IIA", "FlightIIA", "Flight III", "FlightIII"],
        },
        # 这里必须至少有一个核心专名/数量锚点，否则“相控阵+垂发+主炮”只能作为驱逐舰大类共享证据。
        "core_groups": {"aegis_or_spy", "mk41_or_90_96_vls"},
        "min_groups": 2,
    },
    "独立级濒海战斗舰": {
        "category": "护卫舰",
        "demote_below_conf": 0.86,
        "exact": ["独立级", "Independence", "LCS-2", "LCS2"],
        "groups": {
            "trimaran": ["三体船", "三体结构", "三体", "trimaran"],
            "lcs_or_littoral": ["濒海战斗舰", "近海战斗", "近海作战", "LCS"],
            "fifty_seven_mm": ["57mm", "57毫米", "中小口径主炮"],
            "modular_mission": ["模块化任务", "任务模块", "任务舱", "模块化"],
            "stern_flight_deck": ["艉部直升机甲板", "舰尾直升机甲板", "艉部飞行甲板", "舰尾飞行甲板"],
        },
        # 独立级最关键的是三体船。没有三体船，只凭57mm/直升机甲板/模块化，不应把未知护卫舰闭集成独立级。
        "core_groups": {"trimaran"},
        "min_groups": 2,
    },
}


def v49_8_final_known_label(match_result: Dict[str, Any]) -> str:
    final = match_result.get("final_decision") or {}
    known = match_result.get("known_class_result") or {}
    return clean_text(
...
def v49_8_final_category(match_result: Dict[str, Any]) -> str:
    final = match_result.get("final_decision") or {}
    known = match_result.get("known_class_result") or {}
...
def v49_8_final_confidence(match_result: Dict[str, Any]) -> float:
    final = match_result.get("final_decision") or {}
    known = match_result.get("known_class_result") or {}
    return max(
        v49_1_float(final.get("confidence", 0.0)),
        v49_1_float(known.get("confidence", 0.0)),
    )


def v49_8_has_exact_known_name(label: str, text: str) -> bool:
    rules = V49_8_GUARDED_KNOWN_CLASSES.get(label) or {}
    exact_cues = list(rules.get("exact", []))
    exact_cues.append(label)
    return v49_3_has_any(text, exact_cues)


def v49_8_anchor_groups(label: str, text: str) -> List[str]:
    rules = V49_8_GUARDED_KNOWN_CLASSES.get(label) or {}
...
def v49_8_guarded_class_anchor_passed(label: str, text: str, confidence: float) -> Tuple[bool, Dict[str, Any]]:
    """判断一个已知舰级是否有足够锚点支撑闭集结论。"""
    rules = V49_8_GUARDED_KNOWN_CLASSES.get(label)
    if not rules:
        return True, {"guarded": False, "reason": "label_not_guarded"}

    if v49_8_has_exact_known_name(label, text):
        return True, {"guarded": True, "reason": "explicit_known_class_name", "matched_groups": ["exact_name"]}

    matched_groups = v49_8_anchor_groups(label, text)
    core_groups = set(rules.get("core_groups", set()))
    min_groups = int(rules.get("min_groups", 2))
    has_core = bool(core_groups.intersection(set(matched_groups)))

    passed = has_core and len(matched_groups) >= min_groups
    return passed, {
        "guarded": True,
        "reason": "anchor_passed" if passed else "shared_features_without_required_anchor",
        "matched_groups": matched_groups,
        "core_groups": sorted(list(core_groups)),
        "has_core": has_core,
...
def v49_8_demote_known_to_category_unknown(match_result: Dict[str, Any], category: str, reason: str, debug: Dict[str, Any]) -> Dict[str, Any]:
    """把不可靠的 known_class 结论回退成同大类 category_unknown；后续 v49.7 可继续修大类。"""
    category = clean_text(category or v49_8_final_category(match_result) or "未知")
    old_final = match_result.get("final_decision") or {}
    old_alts = old_final.get("alternatives") if isinstance(old_final, dict) else None

    match_result["category_result"] = {
        "label": category,
        "confidence": old_final.get("confidence", debug.get("confidence", 0.0)),
        "status": "v49_8_demoted_weak_known_class",
        "reason": f"v49.8：{reason}。共享区分特征不足以闭集确认具体已知舰级，回退为类别内未知。",
    }
    match_result["known_class_result"] = None
    match_result["open_set_result"] = {
        "is_unknown": True,
        "unknown_scope": UNKNOWN_OUTPUT_TEMPLATE.format(category=category),
        "reason": f"v49.8：{reason}。保持 open-set 类别内未知。",
    }
    match_result["final_decision"] = {
        "result_type": "category_unknown",
        "primary_category": category,
        "primary_class": None,
        "confidence": old_final.get("confidence", debug.get("confidence", 0.0)),
        "status": "v49_8_shared_feature_guard",
        "message": f"最终判定：{category}类别内未知类。v49.8 将弱闭集结果回退为 open-set。",
        "alternatives": old_alts or old_final.get("alternatives", {}),
    }
    match_result["v49_8_shared_feature_open_set_guard"] = {
        "applied": True,
        "action": "demote_known_class_to_category_unknown",
        "reason": reason,
        **debug,
    }
    return match_result
...
def v49_8_shared_feature_open_set_guard(
    match_result: Dict[str, Any],
    observed_attributes: Optional[Dict[str, Dict[str, Any]]] = None,
) -> Dict[str, Any]:
    """
    v49.8：共享区分特征闭集保护。
    只处理 final_decision 已经是 known_class 的情况；如果闭集证据主要来自共享区分特征，
    且缺少该已知舰级应有的核心锚点，则回退成类别内未知。
    """
    if not isinstance(match_result, dict):
        return match_result

    final = match_result.get("final_decision") or {}
    if final.get("result_type") != "known_class":
        match_result["v49_8_shared_feature_open_set_guard"] = {"applied": False, "reason": "not_known_class"}
        return match_result

    label = v49_8_final_known_label(match_result)
    category = v49_8_final_category(match_result)
    confidence = v49_8_final_confidence(match_result)
    text = v49_3_observed_to_text(observed_attributes)

    # 低置信 known_class 且没有显式舰级名称，先作为开放集保守处理。
    # 这一步很窄，只处理 <0.60 的弱闭集结果，防止 unknown 被低分硬拉到已知类。
    if confidence < V49_8_GENERAL_LOW_CONF_DEMOTE and not v49_8_has_exact_known_name(label, text):
        return v49_8_demote_known_to_category_unknown(
            match_result,
            category,
            "low_confidence_known_class_without_exact_name",
            {
                "label": label,
                "category": category,
                "confidence": round(confidence, 4),
                "threshold": V49_8_GENERAL_LOW_CONF_DEMOTE,
            },
        )
    ...
    if not rules:
        match_result["v49_8_shared_feature_open_set_guard"] = {
            "applied": False,
            "reason": "label_not_in_guard_scope",
            "label": label,
            "category": category,
            "confidence": round(confidence, 4),
        }
    ...
    demote_below_conf = float(rules.get("demote_below_conf", 0.0))
    passed, anchor_debug = v49_8_guarded_class_anchor_passed(label, text, confidence)

    debug = {
        "label": label,
        "category": category,
        "confidence": round(confidence, 4),
        "demote_below_conf": demote_below_conf,
        **anchor_debug,
    }

    # 如果置信度很高但有核心锚点缺失，仍不马上回退，避免误伤真实已知类；
    # 但低于该舰级保护阈值时，缺少核心锚点就说明更像“同大类未知”而不是具体已知类。
    if confidence < demote_below_conf and not passed:
        return v49_8_demote_known_to_category_unknown(
            match_result,
            category,
            "guarded_known_class_missing_required_anchor",
            debug,
        )

    match_result["v49_8_shared_feature_open_set_guard"] = {
        "applied": False,
        "reason": "known_class_anchor_sufficient_or_confidence_high",
        **debug,
    }
    return match_result
...
def hierarchical_class_match(
    ...
    # v49.8：如果 known_class 主要由共享区分特征支撑、缺少具体舰级核心锚点，则回退为类别内未知。
    result = v49_8_shared_feature_open_set_guard(result, observed_attributes)
